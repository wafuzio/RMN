...
def _ensure_nltk():
    # NLTK >= 3.9 may need "punkt_tab"; older versions only need "punkt".
    ...
    for pkg in needed:
        try:
            if pkg == "stopwords":
                ...
                # On some NLTK installs, punkt_tab may not exist; handle gracefully
                nltk.data.find("tokenizers/punkt_tab/english")
        except (LookupError, OSError):
            # Try to download; if it's punkt_tab and unavailable, ignore failure
            try:
                nltk.download(pkg, quiet=True)
            except Exception:
                if pkg == "punkt_tab":
                    # Ignore on older NLTK where punkt_tab isn't present
                    pass
_ensure_nltk()
    ...
    for ad_type, extractor_class in extractors.items():
        ...
        # For Skyscraper ads, look for the specific div with data-testid="monetization/search-page-top" (confirmed in screenshot)
            ...
            for div in skyscraper_divs:
                ...
                if not ad:
                    log("Skyscraper extractor returned None; using fallback capture")
                    ad = {
                        'type': 'Skyscraper',
                ...
                # Generate image_path for Skyscraper (similar to TOA logic)
                try:
                    if timestamp and search_term and client:
                        from filename_utils import generate_ad_filename
                        from datetime import datetime
                        ...
                        # Canonical: mirror to image_path
                        ad['image_path'] = ad['skyscraper_image_path']
                        log(f"Generated skyscraper_image_path: {ad['skyscraper_image_path']}")
                except Exception as e:
                    log(f"Error generating Skyscraper filename: {e}")
                    import traceback
                    traceback.print_exc()
        ...
        # For CuratedCarousel ads
            ...
            for idx, div in enumerate(carousel_divs, start=1):
                # Store the raw HTML for image capture
                raw_html = str(div)
                ...
                if not ad:
                    # Fallback carousel capture
                    log("Carousel extractor returned None; using fallback capture")
                    ad = {
                        'type': 'CuratedCarousel'
                    }
                    # Try to capture a visible header
