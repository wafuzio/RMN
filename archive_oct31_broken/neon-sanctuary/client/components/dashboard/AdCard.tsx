...
function AdMedia({ imageUrl, videoUrl, posterUrl, alt, isTOA, isSBA, adType }: { imageUrl?: string; videoUrl?: string; posterUrl?: string; alt?: string; isTOA?: boolean; isSBA?: boolean; adType?: string }) {
  // Prefer video if available, otherwise use image, then poster
  ...
  return (
    <div id="image-frame" className={cn("image-frame w-full overflow-hidden rounded-t-lg bg-gray-100 flex items-center justify-start relative", heightClass)}>
      <img
        ...
        alt={alt || 'ad'}
        className="h-full w-full object-cover"
        style={{ display: error ? 'none' : 'block', objectPosition, contain: 'content' }}
        crossOrigin="anonymous"
        ...
        onError={(e) => {
          ...
          if (!img.src.includes('/api/image/placeholder')) {
            ...
            const label = encodeURIComponent(imageUrl || 'missing');
            const base = (window as any).AD_BASE || '';
            img.src = `${base}/api/image/placeholder?text=${label}`;
            setError(false); // Reset error state to try placeholder
          } else {
            // Placeholder also failed, show error state
            console.error('[AdMedia] Placeholder also failed:', src);
            setLoaded(false);
            setError(true);
          }
        }}
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onOpen(ad)} className={cn("text-left select-none w-full block")} style={{ touchAction: 'none' }}>
        <AdMedia imageUrl={ad.imageUrl} videoUrl={ad.videoUrl} posterUrl={ad.posterUrl} alt={`${ad.brand} ad`} isTOA={ad.adType === "TOA"} isSBA={ad.adType === "SBA"} adType={ad.adType} />
        <div id="content-frame" className={cn("card-text w-full p-4 flex flex-col")}>
          <div className="flex items-center justify-between mb-1 w-full">
            <div className="font-bold text-[1.2em] text-[#111827]">{ad.brand || "Unknown Brand"}</div>
            <div className="relative">
              <Badge className={cn("pill", TYPE_STYLES[ad.adType || ''] || "bg-gray-200 text-gray-800 border-none")}>{ad.adType?.toLowerCase() === "sba" || ad.adType?.toLowerCase() === "sbv" ? ad.adType.toUpperCase() : ad.adType?.replace(/_/g," ") || 'Unknown'}</Badge>
