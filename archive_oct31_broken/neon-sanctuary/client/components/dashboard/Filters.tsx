...
export function Filters({
  ...
  // const selectedBrands = new Set(value.brands || []);

  // Determine which clients to fetch keywords from
  const selectedClients = value.clients && value.clients.length > 0 ? value.clients : [];
  const allClientsAvailable = clients || [];
  const isAllClientsSelected = selectedClients.length > 0 && selectedClients.length === allClientsAvailable.length;

  // When all clients are selected, use "all"; otherwise use up to 3 selected clients
  const clientsForKeywords = isAllClientsSelected
    ? ["all"]
    : (selectedClients.length > 0 ? selectedClients : []);

  // Pad to 3 to maintain consistent hook calls
  const paddedClients = [...clientsForKeywords];
  while (paddedClients.length < 3) paddedClients.push("");

  // Only pass selected brands to filter keywords (if any brands are selected)
  const brandsForFilter = value.brands && value.brands.length > 0 ? value.brands : undefined;

  const keywordsQuery1 = useAds({
    retailer,
    client: paddedClients[0] || "",
    brands: brandsForFilter,
    pageSize: 200,
  });
  const keywordsQuery2 = useAds({
    retailer,
    client: paddedClients[1] || "",
    brands: brandsForFilter,
    pageSize: 200,
  });
  const keywordsQuery3 = useAds({
    retailer,
    client: paddedClients[2] || "",
    brands: brandsForFilter,
    pageSize: 200,
  });
  ...
  const availableKeywords = useMemo(() => {
    ...
    // Merge keywords from all queries
    for (const query of [keywordsQuery1, keywordsQuery2, keywordsQuery3]) {
      const pages = query.data?.pages || [];
      for (const p of pages) {
        for (const c of p.cards || []) {
          if (c.keyword?.trim()) {
            set.add(c.keyword.trim());
          }
        }
      }
    }
  ...
  }, [keywordsQuery1.data, keywordsQuery2.data, keywordsQuery3.data]);
  ...
  return (
    <div className="card-surface p-4 flex flex-wrap items-end gap-3">
      {/* BRANDS FILTER - COMMENTED OUT FOR SIMPLIFICATION */}
      {/* <div className="flex-1 min-w-[220px]">
        <label className="block text-sm text-[#6b7280] mb-1">Brands</label>
                  ...
                  <CommandItem
                    ...
                    onSelect={() => {
                      if (selectedBrands.size === brands.length) {
                        ...
                        onChange({ ...value, brands: brands.slice() });
                      }
                    }}
                  >
                    <Checkbox checked={selectedBrands.size === brands.length} aria-label="Select all brands" />
                  ...
                  {brands.map((b) => (
                    <CommandItem
                      ...
                      onSelect={() => {
                        ...
                        onChange({ ...value, brands: Array.from(next) });
                      }}
                    >
                      <Checkbox checked={selectedBrands.has(b)} aria-label={`toggle ${b}`} />
        ...
        <Popover open={openKeywords} onOpenChange={setOpenKeywords}>
          <PopoverTrigger asChild>
            <div
              ...
              tabIndex={0}
              className={cn(
                "w-full min-h-[38px] px-3 py-2 text-left border rounded-md bg-white hover:bg-gray-50 cursor-pointer",
                selectedKeywords.size ? "flex flex-wrap gap-2" : ""
              )}
              aria-haspopup="listbox"
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setOpenKeywords(true); } }}
            >
