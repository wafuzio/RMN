...
def screenshot_instacart_ads(json_path: str, html_path: str, output_dir: str, profile_dir: str = None, headless: bool = True):
    ...
    with sync_playwright() as p:
        ...
        # Screenshot ad containers using structural selectors
        print("\n   Querying ad containers...")
        
        # Find carousel regions and walk up to border containers
        regions = page.query_selector_all('[role="region"][aria-label*="carousel"]')
        print(f"   Found {len(regions)} carousel regions")
        
        shoppable_ads = []
        for region in regions:
            try:
                # Anchor on the carousel inside the region
                carousel = region.query_selector('[data-testid="shoppable-list-sliding-carousel"]')
                if not carousel:
                    continue
                
                # Use evaluate_handle to walk up from carousel to -inner ancestor, then to its parent
                try:
                    card = carousel.evaluate_handle('''(el) => {
                        let current = el;
                        // Walk up to find div[id$="-inner"]
                        while (current && current.parentElement) {
                            current = current.parentElement;
                            if (current.id && current.id.endsWith('-inner')) {
                                // Found -inner, return its parent (the border container)
                    ...
                    if card:
                        shoppable_ads.append(card.as_element())
                except Exception as inner_e:
                    print(f"   ⚠️ Could not find card from carousel: {inner_e}")
                    continue
