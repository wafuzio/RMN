            ...
            for ad_type, elem in elements:
                ...
                try:
                    ...
                    if advertiser:
                        ...
                        if logo_db:
                            try:
                                # Find the BRAND LOGO specifically (not product images)
                                # The brand logo is in the link to /brands/
                                brand_link = elem.query_selector('a[href*="/brands/"]')
                                if brand_link:
                                    logo_img_elem = brand_link.query_selector('img')
                                    if logo_img_elem:
                                        logo_src = logo_img_elem.get_attribute('src')
                                        if logo_src and 'display.instacart.com' in logo_src:
