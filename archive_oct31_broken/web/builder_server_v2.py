...
def api_ads_cards():
    ...
    Note: By default, cards without resolvable images are excluded. Set include_unresolved=1 to include them
    ...
    if start_date:
        ...
        print(f"[{retailer}] Filtering by date range: {start_date} to {end_date or 'today'}")
        ...
        all_cards = filtered_cards
        print(f"[{retailer}] After date filtering: {len(all_cards)} cards remain")
    else:
        # No date range filtering - return all cards (lifetime)
        print(f"[{retailer}] No date filter specified (lifetime mode) - returning all {len(all_cards)} cards")
