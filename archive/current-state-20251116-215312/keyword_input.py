...
class KeywordInputApp:
    ...
    def __init__(self, root):
        ...
        ttk.Label(paths_frame, text="Profile dir (root):", style='TLabel').grid(row=0, column=0, sticky="w", pady=(0,5))
        self.profile_dir_var = tk.StringVar(value=os.path.expanduser("~/ChromeProfiles"))
    ...
    def _run_scraper_for_retailer(self, retailer_name, retailer_slug, adapter, folder_name, keywords):
        ...
        # Derive a per-retailer Chrome profile directory from the configured root
        profile_root = os.path.expanduser(self.profile_dir_var.get().strip())
        profile_dir = os.path.join(profile_root, retailer_slug)
        if profile_dir and not os.path.isdir(profile_dir):
            try:
                os.makedirs(profile_dir, exist_ok=True)
            except Exception:
                self.log(f"[{retailer_name}] profile dir not found or not creatable: {profile_dir} (continuing without persistent profile)")
                profile_dir = None
        ...
        # Wire through the adapter's expected env var so legacy scripts see it
        if adapter.profile_env and ctx.profile_dir:
            os.environ[adapter.profile_env] = ctx.profile_dir
        ...
        for i, keyword in enumerate(keywords):
            ...
            while retry_count < max_retries and not scraped:
                ...
                try:
                    ...
                    self.log(f"[{retailer_name}] attempt {retry_count+1}/{max_retries} for '{keyword}' starting adapter.search_and_capture")
                    ...
                    # Heuristic: if adapter signaled not-ok but we actually produced new run_results_*.json,
                    # treat as success to avoid pointless retries (e.g., Walmart GUI).
                    if not ok and not bail:
                        try:
                            current_json = set(glob.glob(os.path.join(runs_dir, "run_results_*.json")))
                            new_json = [p for p in current_json - baseline_json if os.path.getmtime(p) >= run_start_ts]
                            if new_json:
                                ok = True
                                reason = (reason or "") + " [json_detected_by_gui]"
                                self.log(f"[{retailer_name}] adapter returned not-ok but GUI detected new JSON -> treating as success ({len(new_json)} new)")
                        except Exception:
                            pass

                    self.log(f"[{retailer_name}] attempt {retry_count+1}/{max_retries} for '{keyword}' returned: res={res!r} -> ok={ok} bail={bail} reason={reason}")
                    ...
                    if bail:
                        ...
                        self.log(f"[{retailer_name}] bail=true for '{keyword}' on attempt {retry_count+1}: {msg}")
                    ...
                    retry_count += 1
                    self.log(f"[{retailer_name}] exception on attempt {retry_count}/{max_retries} for '{keyword}': {e}")
