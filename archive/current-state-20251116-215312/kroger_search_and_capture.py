...
def search_and_capture(search_term=None, output_dir=None):
    ...
    try:
        ...
        pass
    debug_log = os.path.join(runs_dir, f"capture_debug_kroger_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    log("bootstrap: start")
    log(f"debug: search_term={search_term} output_dir={output_dir}")
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                if not is_still_logged_in:
                    ...
                    log("fatal: session_lost_during_search")
                    return False
                ...
                log(f"main: screenshot -> {screenshot_path} exists={os.path.exists(screenshot_path)}")
                            ...
                            try:
                                ...
                                if not featured_flag:
                                    ...
                                    if not featured_badge:
                                        ...
                                        log(f"carousel: skip {i+1} not featured")
                                        continue
