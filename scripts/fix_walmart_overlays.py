#!/usr/bin/env python3
"""
Fix Walmart video overlays using auto_detect_video_overlay.py CV detection.
...
from scripts.auto_detect_video_overlay import detect_walmart_sbv_bounds
...
def process_json_file(json_path: Path, client_root: Path, dry_run: bool = False) -> int:
    ...
    for ad in ads:
        ...
        # Only process SBV ad types
        if 'sbv' not in ad_type.lower() and 'sponsored_brand_video' not in ad_type.lower():
            continue
        
        # Get image path
        image_path_rel = ad.get('image_path') or ad.get('screenshot')
        if not image_path_rel:
            continue
        
        # Build full image path - handle different path formats
        ...
        if not image_path.exists():
            ...
            image_path = Path(__file__).parent.parent / "output" / image_path_rel
        
        if not image_path.exists():
            print(f"  ⚠️ Image not found: {image_path_rel}")
            continue
        
        # Run CV detection
        bounds = detect_walmart_sbv_bounds(image_path)
        if not bounds:
        ...
        old_width = old_vo.get('width', 'N/A')
        
        if dry_run:
            print(f"  Would update {brand}: width={old_width}→{bounds['width']}")
        else:
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Fix Walmart video overlays with CV detection")
    ...
    base_dir = Path(__file__).resolve().parent.parent / "output" / "walmart"
