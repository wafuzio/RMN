...
def _build_slots_array(matched_results, json_ads):
    ...
    image_path points to assets/target/product_images/<tcin>.<ext> —
    downloaded on first encounter, re-linked on subsequent appearances of the
    same tcin (same product can appear multiple times on one page).
    """
    try:
    ...
    for i, (slot, json_ad) in enumerate(matched_results):
        ...
        if _img_store and product_id:
            ...
            elif has_image('target', product_id):
                image_path = get_image_path('target', product_id)
                _image_cache[product_id] = image_path
            elif image_url:
                image_path = download_and_store('target', product_id, image_url)
                _image_cache[product_id] = image_path
        ...
        entry = {
            ...
            'ad_type': ad_type,
            'is_sponsored': d.get('is_sponsored', ad_type in ('Sponsored_Product', 'ListingPageBannerAd', 'Sponsored_Logo')),
