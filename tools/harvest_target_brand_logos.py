#!/usr/bin/env python3
"""Harvest Target brand logos from existing run_results JSONs.

This is a post-processing tool. It scans:

- output/target/**/runs/run_results_*.json

For each ad:
- Picks a brand name (brand field, already canonicalized where possible)
- Picks a logo/image URL (currently image_url from banner/logo creatives)
- Calls BrandLogoDatabase.add_brand_logo(brand, logo_url, retailer="target", metadata=...)

Usage:
    python3 tools/harvest_target_brand_logos.py
"""
...
def iter_run_results() -> Path:
    """Yield all Target run_results JSON paths under output/target/**/runs/"""
...
def pick_brand(ad: Dict[str, Any]) -> Optional[str]:
    ...
    For Target we currently only have `brand`, but keep the same
    pattern as Amazon (prefer canonical if present in future).
    """
...
def pick_logo_url(ad: Dict[str, Any]) -> Optional[str]:
    ...
    Target does not currently emit a dedicated brand_logo_url; the
    creative image lives in image_url for both banners and logos.
    We still keep the same priority structure as Amazon in case we
...
def main() -> None:
    ...
    for json_path in iter_run_results():
        ...
        retailer = doc.get("retailer") or "target"
        if retailer != "target":
            continue
    ...
    print("=== Target Brand Logo Harvest ===")
