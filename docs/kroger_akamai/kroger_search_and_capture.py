...
from kroger_step_logger import (
    StepLogger, step, attach_network_listeners,
    log_navigator_diagnostics, log_webgl_diagnostics, log_akamai_trip,
)
...
def _launch_context_resilient(pw, client_name: str):
    ...
    for cand in candidates:
        try:
            ...
            # CRITICAL: Override navigator.webdriver to false (not undefined)
            # HAR analysis showed undefined was wrong - real Chrome reports false
            # Use delete + redefine as data property to avoid detectable getter
            ctx.add_init_script("""
...
def search_and_capture(search_term=None, output_dir=None):
    ...
    # Initialize granular step logger (JSONL telemetry for Akamai debugging)
    ...
    with single_browser_lock(timeout=600):
        with sync_playwright() as p:
            ...
            try:
                ...
                # Attach network event listeners for continuous telemetry
                ...
                try:
                    ...
                    if is_blocked:
                        log_akamai_trip(SL, f"homepage_{reason}", url=page.url, details=details)
                ...
                try:
                    ...
                    if _box:
                        ...
                        is_blocked, reason, details = diag.check_akamai_block(page)
                        if is_blocked:
                            log_akamai_trip(SL, f"search_results_{reason}", url=page.url, details=details)
                ...
                if not _search_typed:
                    ...
                    is_blocked, reason, details = diag.check_akamai_block(page)
                    if is_blocked:
                        log_akamai_trip(SL, f"direct_nav_{reason}", url=page.url, details=details)
                ...
                try:
                    ...
                    context.close()
                    SL.log("browser_closed",
                           sensor_posts=SL.sensor_posts,
                           rum_beacons=SL.rum_beacons,
                           blocks=SL.blocks)
                    SL.log("session_end",
                           abck_snapshots=len(SL._abck_history),
                           jsonl_path=SL.path)
