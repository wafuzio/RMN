...
**2025-10-08:** Comprehensive forensics and bail system. **Status: Still flagged by PX (0% success rate)**. Implemented run report system (run_report.json + run_report.md) with outcome tracking, timings (to_home_ms, after_submit_px_ms, results_ready_ms), environment info (UA, WebGL), cookie persistence stats, PX solver stats (tries/cycles/cleared), network forensics (req_failed, resp_doc, route_errors). Added bail system: adapter returns `{'ok': bool, 'bail': bool, 'reason': str}` to stop blind retries on non-retryable failures (px_locked, hard_block, fatal). Fixed net_counters scope issue (passed to _launch()). Added resilient homepage navigation with 3-tier fallback (domcontentloaded → commit+search → networkidle). Debug tools: Break on PX modal, break on /blocked, line trace, Playwright trace viewer, steps.jsonl with microsecond timestamps. All artifacts tracked in report. (PR #TBD)

**2025-10-06 (Final):** PerimeterX evasion implementation with comprehensive telemetry. Removed heavy anti-fingerprinting (playwright-stealth only), removed homepage scrolling (major trigger), added dwell time after typing (400-1100ms), prefer button click over Enter, micro caret movement, immediate retry controller (3 quick retries for same widget), longer steady holds (6.8-10.2s adaptive), off-domain guard (nav-only), JSONL step logger with millisecond-precision telemetry. Human-like behaviors: variable keystroke delays (80-220ms), micro-mouse attention movements, drift reading after scroll, back-scroll peek (35% chance), random product hover. Still being flagged by PX. (PR #TBD)

**2025-10-06 (Initial):** Complete PerimeterX bypass implementation. Native wheel scrolling (replaced JS scrollTo), auto press-and-hold solver (3.1-3.6s), PX cookie verification (_px3/_pxvid), modal + /blocked detection, off-domain guard rails (blocks Google), telemetry (tracing/video/crash hooks), stable per-profile fingerprint, playwright-stealth integration, empty Chrome args (no banners), chromium_sandbox=True. (PR #TBD)
...
**2025-10-08:** Documentation cleanup and organization. Removed duplicate files (walmart_ad_html copy.md), created docs/reference/ for large HTML samples (Instacart_ad_html.md 160K, Walmart_ad_html.md), created docs/archive/ for completed/old docs (REPO_REORG_SUMMARY, PLAYWRIGHT_BOOTSTRAP, test directories). Moved FILE_TREE.md (445K auto-generated) to root. Updated CONTEXT_SEED.md with honest Walmart status (WIP - PerimeterX challenges, 0% success rate), documented all debug tools, added bail system and run report documentation. (PR #TBD)
