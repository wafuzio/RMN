"""Typed exception hierarchy for cli-web-walmart.
...
class WalmartError(Exception):
    """Base exception for all cli-web-walmart errors."""
...
class RateLimitError(WalmartError):
...
class NetworkError(WalmartError):
    """Connection failed -- DNS resolution, TCP connect, TLS handshake."""


class ServerError(WalmartError):
...
class NotFoundError(WalmartError):
    """Resource not found (HTTP 404)."""


class RPCError(WalmartError):
...
def _error_code_for(exc: WalmartError) -> str:
