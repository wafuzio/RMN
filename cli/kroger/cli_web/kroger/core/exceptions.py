"""Typed exception hierarchy for cli-web-kroger.
...
class KrogerError(Exception):
    """Base exception for all cli-web-kroger errors."""
...
class RateLimitError(KrogerError):
...
class NetworkError(KrogerError):
    """Connection failed -- DNS resolution, TCP connect, TLS handshake."""


class ServerError(KrogerError):
...
class NotFoundError(KrogerError):
    """Resource not found (HTTP 404)."""


class RPCError(KrogerError):
...
def _error_code_for(exc: KrogerError) -> str:
